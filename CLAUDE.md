...
**スコープと優先順位（Iter 0）:** 本リポジトリ（**そらもよう**）内の実装・ドキュメント作業向け。`~/.claude/CLAUDE.md` の汎用ルールと食い違う場合は、**本書と `docs/` を優先**（ブランチ戦略・Firebase・画像圧縮・日本語コミット等）。
...
# 3. PR 作成（`gh` コマンド、または GitHub MCP が使える環境で）
```
