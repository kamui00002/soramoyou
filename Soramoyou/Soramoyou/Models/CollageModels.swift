...
//
//  機能: v1「配置写真（朝・昼・夕・夜などを並べる）」/ v2「広角合成（無料・OpenCVで横に繋ぐ）」の共通データ。
...
///
/// `.collage`=配置写真（タイル状に並べる・合成しない・必ず成功・無料）、
/// `.panorama`=広角合成（OpenCV で横に1枚に繋ぐ・無料）。種別を持つことで
/// 「再編集を出さない」「抽出メタを付けない」等の分岐と、図鑑の集計を一貫させる。
