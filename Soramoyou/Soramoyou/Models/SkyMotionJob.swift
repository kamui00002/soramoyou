...
///
/// 較正の根拠（実測。動き量 = 隣接フレーム平均絶対差 × fps・継ぎ目1.2秒を除いた本体中央値）:
///   - 旧quick(横写真) 幅1.77% ÷ 5.64s = 0.314%/秒 → 実測12.4「とても良かった」
///   - 旧calm (横写真) 幅1.67% ÷10.64s = 0.157%/秒 → 実測 6.4「動いていない」
///   - 旧quick(縦写真) 幅2.36% ÷ 5.00s = 0.472%/秒 → 実測11.8（プレビューで承認済み）
///   見かけの動きは (幅比 ÷ 出力尺) にほぼ完全比例する（予測と実測の誤差3%）。
///
/// 比率の決め方: **どの向きの写真でも、過去に承認された設定より遅くしない**。
///   旧コードは絶対pxだったので同じプリセットでも縦写真の方が比率が大きく（例 quick: 縦2.36% / 横1.77%）、
///   低い方（横）に揃えると縦写真が25%遅くなる＝承認済みの体感を下げる退行になる。
///   そこで**縦写真側の比率に揃える**。結果、最も遅い calm でも 0.310%/秒 ≈ 実測12.3 となり、
///   「とても良かった」判定の 12.4 とほぼ同じ速度が下限として保証される。
enum SkyMotionPreset: String, CaseIterable, Identifiable {
    ...
    /// サーバーの setpts 係数（`functions/skyMotion.js` の `LOOP_DURATION_FACTORS`）から
    /// 決まる出力尺の実測値(秒)。**速さの計算にのみ使う較正用の定数**であり、UI表示には使わない
    /// （表示は `label` の丸めた文言）。
    ///
    /// 見かけの速さ = `driftWidthRatio ÷ approximateSeconds` なので、尺だけ伸ばして
    /// ドリフトを据え置くと速度が落ちて「動いて見えない」に戻る。その回帰を
    /// `SkyMotionAssetPreparerTests` が この値を使って検証する。
