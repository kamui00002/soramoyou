...
/// ループ動画のプリセット（単一3択・2026-07-26確定 / 2026-08-02に速度モデルを訂正）。
///
/// ⚠️ **体感速度を決めているのはサーバーの setpts 係数であって、`driftWidthRatio` ではない。**
///    2026-07-24〜08-01 は「尺＝setpts / 速さ＝trajectory」という前提でチューニングしていたが、
///    **同一写真・同一マスクでの対照実験（2026-08-02）でこのモデルは否定された**:
///      指標＝隣接フレーム平均絶対差 × fps（継ぎ目1.2秒を除いた本体中央値）
///      - loopDuration=short（setpts1.3・ドリフト44px）→ 動き量 **10.48** …… 実機OK
///      - loopDuration=long （setpts2.3・ドリフト**63px**）→ 動き量 **3.04** …… 実機NG「動いてない」
///      ドリフトを1.43倍**増やした**のに3.45倍遅い。＝ trajectory はほとんど効いていない。
///    そのため速度の担保は **サーバー側の setpts 上限**（`skyMotionCore.LOOP_SLOW_FACTOR_MAX`）で行う。
///
/// ⚠️ ドリフト量は **画像幅に対する比率** で持つ（絶対pxではない）。
///    写真は長辺1920に縮小されるため、同じ絶対pxでも横写真(幅1920)は縦写真(幅1440)より
///    相対移動量が1.33倍小さくなり、向きによって指示内容がブレる。効果は薄いと分かったが、
///    「向きで挙動が変わらない」こと自体は保っておく（比率のまま据え置き）。
///
/// 📌 未解決（2026-08-02）: 大きい積雲が動かず、薄い巻雲だけが動く実機FBがある。
///    空マスクをダンプして確認したところ**マスクは正常**（空カバー率92.8%・積雲は白＝動かす側）
///    だったため、原因は Kling 側の挙動と考えられる。まだ写真1枚ぶんの証拠しかない。
///    切り分けの実験: 同じ写真・setpts1.3 のまま比率を 0.08〜0.10 まで上げて効くか見る
///    （幅の4.7%まではゴースト無し・地上固定OKを実証済み。それ以上は未検証）。
enum SkyMotionPreset: String, CaseIterable, Identifiable {
    ...
    ///
    /// ⚠️ この値で体感速度は**変わらない**（enum 冒頭の対照実験を参照）。速度の担保は
    ///    サーバーの setpts 側。ここは「Kling への移動方向・量のヒント」として残している。
