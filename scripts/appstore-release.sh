...
#   scripts/appstore-release.sh prepare --version 1.9.2 --build 72
#   scripts/appstore-release.sh submit --version 1.9.2 --build 72 --yes
...
Examples:
  $(basename "$0") status
  $(basename "$0") prepare --version 1.9.2 --build 72
  $(basename "$0") submit --version 1.9.2 --build 72 --yes
EOF
}
...
case "$SUBCOMMAND" in
    status)
        ...
        ;;
    prepare)
        # チェック順: release_notes → secret → fastlane（オフラインでもプレースホルダ検知だけは検証できる）
        check_release_notes
        ...
        ;;
    submit)
        # チェック順: release_notes → secret → fastlane
        check_release_notes
