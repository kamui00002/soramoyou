...
## 反応してくれた人一覧のクエリ形状（2026-08-25）⭐️

プロフィール →「反応してくれた人を見る」（`Views/ReactedUsersView.swift` /
`ViewModels/ReactedUsersViewModel.swift`）は2段で辿る。

| 手順 | クエリ形状 | 上限 | インデックス |
|---|---|---|---|
| 1 | `posts`: `userId == 自分` + `createdAt DESC` | 60件取得 → 非公開を除外 → 先頭30件 | 既存（`fetchUserPosts`） |
| 2 | `likes`: `postId in [...]` | **30**（`in` の上限）／読み取り 500件 | **不要**（等値1本のみ・単一フィールドの自動インデックスで足りる） |

### ⚠️ `likes` に `order(by:)` を付けない

`postId in [...]` に `order(by: "createdAt")` を足すと複合インデックスが要る。
並べ替えはクライアント側の集約（`ReactedUsersViewModel.aggregate`）で行うので、
**インデックス deploy を増やさない**方を選んでいる。

その代わり `limit(to: 500)` は「**非常弁であって正確な上位 N 件ではない**」。
`order(by:)` が無いため、上限に達したときに切り捨てられるのはドキュメント ID 順の後ろ側で、
新しい順の上位が残る保証はない。`likes` の rules は read が `isAuthenticated()` のみで
list 上限を強制しないため、人気投稿が窓に入ったときに読み取り量が青天井になるのを防ぐのが目的。

⚠️ **上限に張り付くと表示件数が過小になる**。切り捨てられるのは末尾ではなく
ドキュメント ID 順の任意の部分集合なので、全員の「N件のいいね」が同時に目減りし、
人によっては一覧から丸ごと消える。**例外にもエラーにもならない**ため、
`reacted_users_loaded` の `like_count` パラメータ（500 に張り付いていたら要対処）と
`fetchLikes` の警告ログでのみ検知できる。現在のデータ量（全体で 100 件未満）では
到達しないが、投稿が伸びたら**分割取得か件数の別出典**へ切り替えること。

### ⚠️ 「最近の投稿」であることを画面に明示する

`in` の上限30により、この一覧は **直近30件（非公開を除く）の投稿への反応**しか出ない。
それより古い投稿への反応は出ないので、画面の文言でそう名乗ること（全期間の反応が出ると誤解させない）。
